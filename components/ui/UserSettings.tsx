...
import {inviteUserAsParentAction, updateSettingsAction} from "@/server/actions";

import {toast} from "sonner";
export default function UserSettings() {
    const t = useTranslations('user-dashboard');
    const [notification,setNotification] = useState<boolean|undefined>(false);
    const [nsfwTitle, setNSFWTitle] = useState<boolean|undefined>(false);
        ...
        debounce(() => {
            startTransition(async () => {
                ...
                if(result){
                    ...
                    toast.error(t("toast-settings-successfully-updated"));
                }
            });
        }, 500),
    ...
    return (
        <div className="grid gap-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <Switch
                    onCheckedChange={(checked) => { setNSFWTitle(checked); }}
                    checked={nsfwTitle}
                />
            </div>
        </div>
