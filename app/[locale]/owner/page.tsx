...
export  default async() => {
  const {data: activeOrders, error:activeErrorOrder} = await getOwnerOrders('myOrders');
  const {data: completedOrders, error:completedErrorOrder} = await getOwnerOrders('myCompletedOrders');

  return <>
    <OwnerPage activeOrders={activeOrders} completedOrders={completedOrders} />
  </>
}

function OwnerPage({activeOrders=[],completedOrders=[]}:{activeOrders?:ownerOrder[];completedOrders?:ownerOrder[]}) {
