...
import {inviteUserAsParentAction, updateWalletAddressAction} from "@/server/actions";
...
export function UserInvite() {
  ...
  function inviteUser(){
      startTransition(async () => {
          const result = await inviteUserAsParentAction(username);
          if(result){
            toast.success(t("toast-invite-successfully-updated"));
          }else{
              toast.error(t("toast-invite-successfully-updated"));
          }
      });
  }
  ...
  return (
    <section className="mb-4 space-y-4 border-b pb-4">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <Button className="h-11 rounded-2xl px-6" onClick={inviteUser}>Invite</Button>
      </div>
    </section>
  );
}
