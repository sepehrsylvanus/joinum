...
export async function getUserInfos(): Promise<{ data: userInfo, error: errorType }> {
    try {
        const res = await apiClient.get('/api/v1/users/info')
        const {status, data, error} = res.data;

        return {
            data: {...data, account_type: data.account_type.toString().toLowerCase()},
            error: {
                code: error?.code || null,
                message: error?.message || '',
            }
...
export async function getUserBalance(): Promise<{ data: userBalance, error: errorType }> {
    try {
...
export async function updateSettings(settings: settings): Promise<{ data: settings, error: errorType }> {
    try {
...
export async function getOwnerInfos(): Promise<{ data: ownerInfos, error: errorType }> {
    try {
...
export async function getOwnerOrders(orderType: ownerOrderType): Promise<{ data: ownerOrder[], error: errorType }> {
    try {
...
export async function postLogin(initData: string): Promise<{ data: userLogin, error: errorType }> {
    try {
...
export async function postBoomarkList(orders: number[] = []): Promise<{ data: { message: string }, error: errorType }> {
    try {
